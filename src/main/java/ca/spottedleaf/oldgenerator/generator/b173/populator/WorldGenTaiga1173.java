...
public class WorldGenTaiga1173 extends WorldGenerator173 {
    ...
    public WorldGenTaiga1173(int minHeight, int maxHeight) {
                ...
                for (i2 = centerX - l2; i2 <= centerX + l2 && flag; ++i2) {
                    for (j2 = centerZ - l2; j2 <= centerZ + l2 && flag; ++j2) {
                        if (l1 >= minHeight && l1 < (maxHeight + 1)) {
                            type = blockGetter.getBlock(i2, l1, j2);
            ...
            if (!flag) {
                ...
                if ((type == Block.GRASS_BLOCK || type == Block.DIRT) && centerY < 128 - l - 1) {
                    ...
                    for (i2 = centerY + l; i2 >= centerY + i1; --i2) {
                        for (j2 = centerX - l2; j2 <= centerX + l2; ++j2) {
                            ...
                            for (int i3 = centerZ - l2; i3 <= centerZ + l2; ++i3) {
                                ...
                                if ((Math.abs(k2) != l2 || Math.abs(j3) != l2 || l2 <= 0) && !LegacyUtil173.Block_o(blockGetter.getBlock(j2, i2, i3))) {
                                    blockSetter.setBlock(j2, i2, i3, Block.SPRUCE_LEAVES);
                                }
                            }
                        }
                    ...
                    for (i2 = 0; i2 < l - 1; ++i2) {
                        ...
                        if (type.isAir() || BlockConstants.isLeaves(type)) {
                            blockSetter.setBlock(centerX, centerY + i2, centerZ, Block.SPRUCE_LOG);
                        }
                    }
