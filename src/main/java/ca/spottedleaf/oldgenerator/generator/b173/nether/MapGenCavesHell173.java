...
public class MapGenCavesHell173 extends MapGenBase173 {
    ...
    protected void a(Block.Getter blockGetter, Block.Setter blockSetter, int i, int j, double d0, double d1, double d2) {
        this.a(blockGetter, blockSetter, i, j, d0, d1, d2, 1.0F + this.random.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void a(Block.Getter blockGetter, Block.Setter blockSetter, int i, int j, double d0, double d1, double d2, float f, float f1, float f2, int k, int l, double d3) {
        double d4 = i * 16 + 8;
        double d5 = j * 16 + 8;
        float f3 = 0.0F;
        float f4 = 0.0F;
        ...
        for (boolean flag1 = random.nextInt(6) == 0; k < l; ++k) {
            ...
            d0 += MathHelper173.cos(f1) * f5;
            d1 += f6;
            d2 += MathHelper173.sin(f1) * f5;
            if (flag1) {
                f2 *= 0.92F;
            } else {
                f2 *= 0.7F;
            ...
            if (!flag && k == j1 && f > 1.0F) {
                this.a(blockGetter, blockSetter, i, j, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                this.a(blockGetter, blockSetter, i, j, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                return;
            }
                    ...
                    for (j3 = k1; !flag2 && j3 < l1; ++j3) {
                        for (int k3 = k2; !flag2 && k3 < l2; ++k3) {
                            for (int l3 = j2 + 1; !flag2 && l3 >= i2 - 1; --l3) {
                                i3 = (j3 * 16 + k3) * 128 + l3;
                                if (l3 >= 0 && l3 < 128) {
                                    if (LegacyUtil173.getBlockIndex(blockGetter, i3).compare(Block.LAVA)) {
                                        flag2 = true;
                                    }
    ...
    protected void generate(Block.Getter blockGetter, Block.Setter blockSetter, int minHeight, int maxHeight, int i, int j, int k, int l) {
        ...
        for (int j1 = 0; j1 < i1; ++j1) {
            ...
            double d1 = (double) (this.random.nextInt(maxHeight + 1 - minHeight) + minHeight);
            double d2 = (double) (j * 16 + this.random.nextInt(16));
            int k1 = 1;

            if (this.random.nextInt(4) == 0) {
                this.a(blockGetter, blockSetter, k, l, d0, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; ++l1) {
                ...
                this.a(blockGetter, blockSetter, k, l, d0, d1, d2, f2 * 2.0F, f, f1, 0, 0, 0.5D);
            }
        }
    }
}
